...
export const stakeNft = async (wallet: WalletContextState, mint: PublicKey, dwarf: String, occupation: String, season: String, gemstone: String, startLoading: Function, endLoading: Function, updatePageStates: Function, startDisable: Function, endDisable: Function) => {
    startLoading();
    startDisable();
    ...
    try {
        ...
        console.log("KCRWN NFT = ", mint.toBase58(), userTokenAccount.toBase58());
...
export const calculateAvailableReward = async (userAddress: PublicKey) => {
    ...
    const userPoolInfo = {
        ...
        stakedMints: userPool.items.slice(0, userPool.itemCount.toNumber()).map((info) => {
            return {
                ...
                rate: (new anchor.BN(info.rate)).toNumber(),
            }
        }),
    ...
    for (let i = 0; i < userPoolInfo.itemCount; i++) {
        ...
        const rate = userPoolInfo.stakedMints[i].rate;
        
        let reward = 0;
        reward = (Math.floor((now - lastRewardTime) / EPOCH)) * rate * 100;
